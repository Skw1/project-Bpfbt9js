...
`).join("")}async function ae(){try{const e=P;return C.PopularList.insertAdjacentHTML("beforeend",te(e)),i=new y,x(),await e}catch(e){console.log(e.message)}}function x(){let e=T.loadCart();const t=[];"products"in e&&(e=e.products,e.forEach(a=>t.push(a.productId))),P.forEach(a=>{t.includes(a._id)?(i[`btn1${a._id}`].style.display="none",i[`check${a._id}`].style.display="flex"):(i[`check${a._id}`].style.display="none",i[`btn1${a._id}`].style.display="flex")})}let l=JSON.parse(localStorage.getItem("keyword")),$=JSON.parse(localStorage.getItem("category")),d={};const se=document.getElementById("pagination");function ce(e,t,a){const s={totalItems:e,itemsPerPage:t,visiblePages:5,page:a,centerAlign:!0};new Y(se,s).on("afterMove",ne)}async function ne(e){const t=e.page;var a=JSON.parse(localStorage.getItem("filters"))||{};a.page=t,l==""?l=null:l=JSON.parse(localStorage.getItem("keyword")),l==null?$.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?d={page:a.page}:d={category:$.replace(" ","_").replace("/","&").replace(" ","_"),page:a.page}:$.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?d={keyword:l,page:a.page}:d={category:$.replace(" ","_").replace("/","&").replace(" ","_"),keyword:l,page:a.page},console.log(d);const s=await m.products(d);return O(s)}const p=document.querySelector("#dropdownMenu"),q=document.querySelector("#input-filter"),re=document.querySelector("#filterButt"),f=document.querySelector(".category-content"),oe=document.querySelector(".dropdown-toggle"),w="keyword",H="category";let r=JSON.parse(localStorage.getItem(w)),n=JSON.parse(localStorage.getItem(H)),g={};function J(){(r||n)&&(q.value=r,f.textContent=n)}async function B(){let t=(await m.categories()).map(a=>a.replace("_"," ").replace("&","/").replace("_"," "));t.push("Show all"),n!==null?f.textContent=n:f.textContent="Categories",p.innerHTML="",t.forEach(async a=>{const s=document.createElement("div");s.className="dropdown-item",s.textContent=a,s.onclick=async function(){n=a,localStorage.setItem(H,JSON.stringify(n)),z()},p.appendChild(s)})}async function z(){p.style.display=p.style.display==="flex"?"none":"flex",n!==null?f.textContent=n:f.textContent="Categories",setTimeout(function(){p.style.opacity=p.style.opacity==="1"?"0":"1"},10)}function ie(e){const t=e.target.value;t?localStorage.setItem(w,JSON.stringify(t)):localStorage.setItem(w,JSON.stringify(null))}async function le(e){e.preventDefault(),r==""?r=null:r=JSON.parse(localStorage.getItem(w)),r==null?n.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?g={}:g={category:n.replace(" ","_").replace("/","&").replace(" ","_")}:n.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?g={keyword:r}:g={category:n.replace(" ","_").replace("/","&").replace(" ","_"),keyword:r};const t=await m.products(g);return t.results.length===0?L.style.display="flex":L.style.display="none",ce(t.perPage*t.totalPages,t.perPage,t.page),O(t)}J();B();const D=new y;M();V();Q();document.querySelector(".product_card-list").addEventListener("click",K);X();D.PopularList.addEventListener("click",ee);W();B();J();D.discountList.addEventListener("click",U);oe.addEventListener("click",z);q.addEventListener("input",ie);re.addEventListener("click",le);G.addEventListener("submit",R);
