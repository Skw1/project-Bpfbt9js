import{r as y,b as E,g as N,f as m,l as T,e as S,h as j,d as F,i as V,j as U,a as G,p as R}from"./assets/footer-modal-3a28801b.js";import{P as Y}from"./assets/vendor-f89adc3e.js";const L=document.querySelector(".error");new y;let i,k,_={limit:null};async function O(e){document.querySelector(".product_card-list").innerHTML="",e.results.map(a=>document.querySelector(".product_card-list").insertAdjacentHTML("beforeend",A(a)))}function K(e){if(e.target.classList.contains("product_card-item")||e.target!==e.currentTarget){let a=setInterval(function(){I(),x()},1e3);if(setTimeout(function(){clearInterval(a)},1e4),e.target.nodeName==="use"||e.target.nodeName==="BUTTON"||e.target.nodeName==="svg"){const t=e.target.closest(".product_card-item").dataset.productid;E(t),i[`btn2${t}`].style.display="none",i[`check1${t}`].style.display="flex";return}N(e,".product_card-item");return}}function A(e){const{category:a,img:t,name:s,popularity:u,price:h,size:b,_id:c,is10PercentOff:D}=e;return D?`
...
`).join("")}async function ae(){try{const e=P;return C.PopularList.insertAdjacentHTML("beforeend",te(e)),o=new y,x(),await e}catch(e){console.log(e.message)}}function x(){let e=T.loadCart();const a=[];"products"in e&&(e=e.products,e.forEach(t=>a.push(t.productId))),P.forEach(t=>{a.includes(t._id)?(o[`btn1${t._id}`].style.display="none",o[`check${t._id}`].style.display="flex"):(o[`check${t._id}`].style.display="none",o[`btn1${t._id}`].style.display="flex")})}let l=JSON.parse(localStorage.getItem("keyword")),$=JSON.parse(localStorage.getItem("category")),d={};const se=document.getElementById("pagination");function ce(e,a,t){const s={totalItems:e,itemsPerPage:a,visiblePages:5,page:t,centerAlign:!0};new Y(se,s).on("afterMove",ne)}async function ne(e){const a=e.page;var t=JSON.parse(localStorage.getItem("filters"))||{};t.page=a,l==""?l=null:l=JSON.parse(localStorage.getItem("keyword")),l==null?$.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?d={page:t.page}:d={category:$.replace(" ","_").replace("/","&").replace(" ","_"),page:t.page}:$.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?d={keyword:l,page:t.page}:d={category:$.replace(" ","_").replace("/","&").replace(" ","_"),keyword:l,page:t.page},console.log(d);const s=await m.products(d);return O(s)}const p=document.querySelector("#dropdownMenu"),q=document.querySelector("#input-filter"),re=document.querySelector("#filterButt"),f=document.querySelector(".category-content"),ie=document.querySelector(".dropdown-toggle"),w="keyword",H="category";let r=JSON.parse(localStorage.getItem(w)),n=JSON.parse(localStorage.getItem(H)),g={};function J(){(r||n)&&(q.value=r,f.textContent=n)}async function B(){let a=(await m.categories()).map(t=>t.replace("_"," ").replace("&","/").replace("_"," "));a.push("Show all"),n!==null?f.textContent=n:f.textContent="Categories",p.innerHTML="",a.forEach(async t=>{const s=document.createElement("div");s.className="dropdown-item",s.textContent=t,s.onclick=async function(){n=t,localStorage.setItem(H,JSON.stringify(n)),W()},p.appendChild(s)})}async function W(){p.style.display=p.style.display==="flex"?"none":"flex",n!==null?f.textContent=n:f.textContent="Categories",setTimeout(function(){p.style.opacity=p.style.opacity==="1"?"0":"1"},10)}function oe(e){const a=e.target.value;a?localStorage.setItem(w,JSON.stringify(a)):localStorage.setItem(w,JSON.stringify(null))}async function le(e){e.preventDefault(),r==""?r=null:r=JSON.parse(localStorage.getItem(w));let a=window.innerWidth||document.documentElement.clientWidth,t="";a>=1440?t=9:a<1440&&a>768?t=8:t=6,r==null?n.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?g={limit:t}:g={category:n.replace(" ","_").replace("/","&").replace(" ","_"),limit:t}:n.replace(" ","_").replace("/","&").replace(" ","_")=="Show_all"?g={keyword:r,limit:t}:g={category:n.replace(" ","_").replace("/","&").replace(" ","_"),keyword:r,limit:t};const s=await m.products(g);return s.results.length===0?L.style.display="flex":L.style.display="none",ce(s.perPage*s.totalPages,s.perPage,s.page),O(s)}J();B();const z=new y;M();F();Q();document.querySelector(".product_card-list").addEventListener("click",K);X();z.PopularList.addEventListener("click",ee);V();B();J();z.discountList.addEventListener("click",U);ie.addEventListener("click",W);q.addEventListener("input",oe);re.addEventListener("click",le);G.addEventListener("submit",R);
